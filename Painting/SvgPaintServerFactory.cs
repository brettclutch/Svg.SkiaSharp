...
namespace Svg
{
    ...
    {
        private static readonly SvgColorConverter _colourConverter;

        static SvgPaintServerFactory()
        {
            _colourConverter = new SvgColorConverter();
        }
        ...
        public static SvgPaintServer Create(string value, SvgDocument document)
        {
            ...
            else
            {
                ...
                while (!string.IsNullOrEmpty(value))
                {
                    ...
                    else if (value.StartsWith("#")) // Otherwise try and parse as color
                    {
                        switch(CountHexDigits(value, 1))
                        {
                            case 3:
                                servers.Add(new SvgColorServer((Color)_colourConverter.ConvertFrom(value.Substring(0, 4))));
                                value = value.Substring(4).Trim();
                                break;
                            case 6:
                                servers.Add(new SvgColorServer((Color)_colourConverter.ConvertFrom(value.Substring(0, 7))));
                    ...
                    {
                        return new SvgColorServer((Color)_colourConverter.ConvertFrom(value.Trim()));
                    }
                }
        ...
        public override object ConvertTo(ITypeDescriptorContext context, System.Globalization.CultureInfo culture, object value, Type destinationType)
        {
            ...
            {
                //check for none
                if (value == SvgPaintServer.None || value == SvgColorServer.None) return "none";
                if (value == SvgColorServer.Inherit) return "inherit";
                if (value == SvgColorServer.NotSet) return "";

                var colourServer = value as SvgColorServer;
                if (colourServer != null)
                {
                    return new SvgColorConverter().ConvertTo(colourServer.Color, typeof(string));
                }
